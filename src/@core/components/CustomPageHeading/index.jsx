...
const CustomPageHeading = ({ text, total, print, addButton, handlePrint, openModal }) => {
  return (
    <Box sx={{ display: 'flex', width: '100%', gap: '8px', alignContent: 'center', justifyContent: 'space-between' }}>
      ...
      <Box sx={{ display: 'flex', gap: '8px' }}>
        {print ? <CustomSecondaryButton text={'Print'} handlePrint={handlePrint} /> : null}
        {addButton ? <CustomContainButton text={addButton} handleFunction={openModal} /> : null}
      </Box>
    </Box>
  )
}
